...
Page({
  ...
  photoPickerChange: function (e) {
    var that = this;
    if (that.data.photoUrlArr.length>=3){
      wx.showToast({
        title: '最多上传3张',
        icon:'none',
        duration:2000
    ...
    wx.chooseImage({
      ...
      success: function (res) {
        ...
        if (res.tempFilePaths.length > 0) {
            ...
            success: function (res1) {
              ...
              try {
                ...
                if (res1.statusCode == 200 && retData.code == "0") {
                  var arr = retData.data;
                  var arry = that.data.photoids;
                  var photoids = [...arry, ...arr]
                  that.data.photoids = photoids;
          ...
          that.data.photoUrlArr.push(res.tempFilePaths[0])
          that.setData(that.data);
        }
      }
    })
  ...
  deleteIMGbtn: function(e){
    console.log(e);
    var that = this;
    var idnexN = e.currentTarget.dataset.index;
    console.log(idnexN);
    that.data.photoUrlArr.splice(idnexN, 1);
    that.data.photoids.splice(idnexN, 1);
    that.setData(that.data);
  },
  ...
  submit: function () {
    ...
    if (that.data.photoids.length==0) {
      wx.showToast({
        title: '请完善退货信息'
      })
      return false;
    ...
    var obj = {
      ...
      Photo: that.data.photoids
        //["5c066331bb860f0890d711a7", "5c06633dbb860f0890d711aa"]//that.data.photoids
    }
